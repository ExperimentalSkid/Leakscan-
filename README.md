...
- `LIVE_METADATA_ONLY`: current file-like metadata and sufficient target-identity evidence were verified without retrieving the file body.
- `LIVE_RESTRICTED`: host-native metadata confirms the object and its target identity, but access is restricted.
...
- `UNVERIFIED`: a lead or file object was observed, but the available fingerprints do not establish that it is the case target.
- `CURRENT_REFERENCE_ONLY` and `BLOCKED`: never claim that a live file was confirmed.
